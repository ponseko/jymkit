...
See the [Environments](https://ponseko.github.io/jaxnasium/api/Available-Environments/) page for a complete list of available environments.
...
Hyperparameters and training logic live in the algorithm (the *trainer*); the trainable state
lives in the *agent* that `train` returns, together with the trainer that produced it.
...
`train` returns the trained agent along with its training metrics: by default the mean
return of the episodes finishing in each training iteration, one scalar per iteration.
Iterations containing no finished episodes return a NaN.

See the [Algorithms](https://ponseko.github.io/jaxnasium/algorithms/Algorithms/) page for more details on the included algorithms.
